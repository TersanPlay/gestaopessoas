...
export default function FirstAccess() {
  ...
  const handleCompleteFirstAccess = async (e: React.FormEvent) => {
    ...
    try {
      await completeFirstAccess({ matricula, cpf, password });
  ...
  return (
    <div className="corporate-gradient flex min-h-screen items-center justify-center bg-slate-100 p-4">
      ...
      <div className="relative z-10 grid w-full max-w-5xl gap-6 lg:grid-cols-[1.15fr_0.85fr]">
        <Card className="overflow-hidden border-slate-200/80 bg-white/95 shadow-2xl backdrop-blur">
          ...
          <CardHeader className="space-y-4 pb-4">
            ...
            <div className="space-y-1 text-center">
              <CardTitle className="text-2xl font-bold text-slate-900">Primeiro acesso</CardTitle>
            ...
            {step === 'identify' ? (
              ...
              <form onSubmit={handleCompleteFirstAccess} className="space-y-4">
                <div className="rounded-2xl border border-emerald-200 bg-emerald-50 p-4 text-sm text-emerald-700">
                  Matrícula e CPF validados. Defina sua senha local para concluir o primeiro acesso.
                </div>
                <div className="space-y-2">
                ...
                <div className="flex gap-3">
                  ...
                  <Button type="submit" className="flex-1 bg-slate-900 hover:bg-slate-800" disabled={loading}>
        ...
        <Card className="border-slate-800 bg-slate-950 text-slate-100 shadow-[0_24px_60px_-32px_rgba(15,23,42,0.95)]">
          ...
          <CardContent className="space-y-4 p-6">
            <div className="rounded-2xl border border-slate-800 bg-slate-900/70 p-4">
              <p className="text-xs uppercase tracking-[0.22em] text-slate-400">Etapa 1</p>
              <p className="mt-2 text-sm text-slate-100">Validamos apenas matrícula e CPF na API externa.</p>
            </div>
            <div className="rounded-2xl border border-slate-800 bg-slate-900/70 p-4">
              <p className="text-xs uppercase tracking-[0.22em] text-slate-400">Etapa 2</p>
              <p className="mt-2 text-sm text-slate-100">Você escolhe a senha que ficará salva no sistema local.</p>
            </div>
            <div className="rounded-2xl border border-slate-800 bg-slate-900/70 p-4">
              <p className="text-xs uppercase tracking-[0.22em] text-slate-400">Etapa 3</p>
              <p className="mt-2 text-sm text-slate-100">Nos próximos acessos, o login acontece com a senha local.</p>
            </div>

            {profile ? (
              <div className="space-y-4 rounded-[28px] border border-slate-800 bg-slate-900/80 p-5">
                <div className="flex items-center gap-3">
                  <div className="flex h-11 w-11 items-center justify-center rounded-2xl border border-emerald-400/25 bg-emerald-400/10 text-emerald-200">
                    <BadgeCheck className="h-5 w-5" />
                  </div>
                  <div>
                    <p className="text-sm font-medium text-white">{profile.name}</p>
                    <p className="text-xs text-slate-400">Cadastro encontrado na API externa</p>
                  </div>
                </div>
                ...
                <div className="grid gap-3 sm:grid-cols-2">
                  <div className="rounded-2xl border border-slate-800 bg-slate-950/80 p-4">
                    ...
                    <p className="mt-2 text-sm text-slate-100">{detailValue(profile.matricula)}</p>
                  </div>
                  <div className="rounded-2xl border border-slate-800 bg-slate-950/80 p-4">
                    <p className="text-xs uppercase tracking-[0.22em] text-slate-500">CPF</p>
                    <p className="mt-2 text-sm text-slate-100">{formatCpf(profile.cpf)}</p>
                  </div>
                  <div className="rounded-2xl border border-slate-800 bg-slate-950/80 p-4">
                    <p className="text-xs uppercase tracking-[0.22em] text-slate-500">Cargo</p>
                    <p className="mt-2 text-sm text-slate-100">{detailValue(profile.cargo)}</p>
                  </div>
                  <div className="rounded-2xl border border-slate-800 bg-slate-950/80 p-4">
                    <p className="text-xs uppercase tracking-[0.22em] text-slate-500">Lotação</p>
                    <p className="mt-2 text-sm text-slate-100">{detailValue(profile.lotacao)}</p>
                  </div>
                </div>
