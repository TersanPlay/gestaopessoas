...
import { Calendar as CalendarIcon, Clock, User, Building, MapPin, List, ChevronLeft, ChevronRight, Search } from "lucide-react";
...
const Agenda = () => {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const pendingVisits = response.data
  ...
  const groupVisitsByDate = (sourceVisits: Visit[]) => {
  ...
  const selectedDateVisits = selectedDate ? getVisitsForDay(selectedDate) : [];
    ...
    <div className="space-y-6 animate-fade-in">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground mt-1">Proximas visitas agendadas.</p>
        </div>
      ...
      <div
        className="relative max-w-md cursor-text"
      ...
      >
        ...
        <Input
          ...
          className="pl-9"
          placeholder="Filtrar visitas por visitante, documento ou local..."
        />
      </div>
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center justify-center py-12 text-muted-foreground bg-muted/20 rounded-xl border border-dashed">
          ...
          <p className="text-lg font-medium">{searchTerm ? 'Nenhuma visita encontrada' : 'Nenhuma visita agendada'}</p>
          <p className="text-sm">
            {searchTerm ? 'Tente ajustar o filtro para ampliar os resultados.' : 'As novas visitas agendadas aparecerao aqui.'}
          </p>
        </div>
      ) : (
        <>
              ...
              {sortedDates.map((dateKey) => (
                <div key={dateKey} className="space-y-4">
                  <div className="flex items-center gap-2">
                    <h2 className="text-xl font-semibold capitalize text-primary">{getDayLabel(dateKey)}</h2>
                      ...
                      return (
                        <div
                          ...
                          className={`
                            ...
                            ${!isCurrentMonthDay ? 'opacity-50 bg-muted/10' : 'bg-background'}
                            ${isSelected
                        ...
                        >
                          ...
                          <div className="space-y-1">
                            ...
                            {dayVisits.length > 3 && (
                              <div className="text-[10px] text-muted-foreground text-center">+{dayVisits.length - 3} mais</div>
                            )}
                          </div>
                        </div>
                      );
              ...
              <div className="space-y-4">
                <div className="flex items-center justify-between">
                  ...
                  {selectedDateVisits.length > 0 && (
                    <span className="text-sm text-muted-foreground">{selectedDateVisits.length} visita(s)</span>
                  )}
                </div>
                        ...
                        <Card key={visit.id} className="card-corporate overflow-hidden transition-all duration-200 hover:shadow-md hover:bg-gradient-to-r hover:from-background hover:to-muted/20">
                          <div className="flex">
                            <div className="w-16 bg-muted/50 flex flex-col items-center justify-center border-r p-2 text-center">
                              <span className="text-lg font-bold text-primary">{format(visitDate, 'HH:mm')}</span>
                            </div>
                            <div className="p-3 flex-1 min-w-0">
                              <h4 className="font-medium truncate">{visit.visitor.name}</h4>
                              <p className="text-xs text-muted-foreground truncate mb-2">Doc: {visit.visitor.document}</p>
